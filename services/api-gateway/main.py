...
async def stats(user=Depends(verify_firebase_token)):
    ...
    # Helper function to run synchronous Firestore queries
    def get_jobs_count():
    ...
    # ⚡ Bolt Optimization: Run independent synchronous DB calls concurrently
    # to avoid blocking the event loop and to reduce total endpoint latency.
