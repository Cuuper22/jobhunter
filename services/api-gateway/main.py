...
async def stats(user=Depends(verify_firebase_token)):
    ...
    # ⚡ Bolt Optimization: Parallelize Firestore stats aggregations
    # Instead of blocking the event loop with 4 sequential synchronous .get() calls,
    # we offload them to separate threads and run them concurrently.
    # This reduces the total query time from sum(queries) to max(queries).
