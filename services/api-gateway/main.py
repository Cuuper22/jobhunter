...
async def stats(user=Depends(verify_firebase_token)):
    ...
    # ⚡ Bolt: Run independent synchronous gRPC Firestore queries concurrently
    # Offloading to threads prevents blocking the main event loop and
    # reduces total response time from O(n) to O(1) query latency.
    def get_jobs_count():
