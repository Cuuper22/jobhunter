...
def run_search(config: SearchConfig) -> list[Job]:
    ...
    # Pre-fetch existing URLs to prevent N+1 Firestore queries
    all_urls = [str(url) for url in df["job_url"].tolist() if str(url)]
    ...
    if jobs_to_save:
        save_jobs_batch(jobs_to_save)
        new_jobs.extend(jobs_to_save)
