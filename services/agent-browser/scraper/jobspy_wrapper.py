...
from shared.firestore_client import job_exists, save_job, log, get_existing_job_urls, save_jobs_batch
...
def run_search(config: SearchConfig) -> list[Job]:
    ...
    # Extract all valid URLs, using a set to deduplicate before chunking
    urls_to_check = list({str(url) for url in df["job_url"].tolist() if url})

    # Check which URLs already exist in a batched way
    existing_urls = get_existing_job_urls(urls_to_check)
    ...
    for _, row in df.iterrows():
        ...
        # Add to existing_urls to prevent intra-batch duplicates
