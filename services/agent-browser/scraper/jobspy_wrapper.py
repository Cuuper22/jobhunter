...
from shared.firestore_client import get_existing_jobs, save_jobs, job_exists, save_job, log
...
def run_search(config: SearchConfig) -> list[Job]:
    ...
    urls_to_check = [str(row.get("job_url", "")) for _, row in df.iterrows() if str(row.get("job_url", ""))]
    existing_urls = get_existing_jobs(list(set(urls_to_check)))
    ...
    for _, row in df.iterrows():
        ...
        new_jobs.append(job)
        # Add to existing_urls to prevent duplicates within the same scrape run
